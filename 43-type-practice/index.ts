...
interface IphonesManufacturedAdterDate extends IMobilePhone {
    initialData: string;
}
...
function filterPhonesByDate(
    ...
    return phones
        ...
        .map((phone) => {
            const newObj = { ...phone, initialData: initial };
            return newObj;
        });
}

function filterPhonesByDateGeneric<T>(
    phones: T[],
    key: keyof T,
    initial: string
): Partial<IphonesManufacturedAdterDate>[] {
    return phones
        .filter((phone) => {
            const manufactured = phone[key];
            if (
